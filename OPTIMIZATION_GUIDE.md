...
```bash
# 每天早上8点生成日报并通过邮件发送
0 8 * * * cd /home/ubuntu/copybot && .venv/bin/python scripts/trading_report.py --period today > /tmp/daily_report.txt 2>&1
```
...
Type=simple
User=ubuntu
WorkingDirectory=/home/ubuntu/copybot
ExecStart=/home/ubuntu/copybot/.venv/bin/python /home/ubuntu/copybot/scripts/health_check.py --alert --continuous --interval 10
Restart=always
RestartSec=60
...
[INFO] Target: ubuntu@3.38.98.169:~/copybot
...
[INFO] Tip: Monitor logs with:
  ssh -i ... ubuntu@3.38.98.169 "cd ~/copybot && ./scripts/manage_copy_trader.sh tail app"
```
...
# 查看状态
cd ~/copybot && ./scripts/manage_copy_trader.sh status
